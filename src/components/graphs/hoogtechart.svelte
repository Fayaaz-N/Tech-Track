<script>
    ...
    // deze props worden genegeerd in de chart zelf, we rekenen opnieuw:
    ...
    // Gemiddelde hoogte in meters, met sanity filter (1–3m)
    function berekenGemHoogte(list) {
        const waarden = (list || [])
            .map((v) => v.hoogteMeter ?? v.hoogte_meter ?? null)
            .filter((x) => typeof x === 'number' && !Number.isNaN(x))
            .filter((x) => x >= 1.0 && x <= 3.0); // alleen realistische personenauto-hoogtes
    ...
    // alles afleiden: gemiddelde hoogtes + schaal + posities
    $: {
        avgOud = berekenGemHoogte(voertuigenOud);
        avgNieuw = berekenGemHoogte(voertuigenNieuw);

        if (!avgOud && !avgNieuw) {
            barData = [];
        } else {
            // als één van de twee ontbreekt, spiegel aan de ander zodat de schaal niet crasht
            if (!avgOud && avgNieuw) avgOud = avgNieuw;
            if (!avgNieuw && avgOud) avgNieuw = avgOud;

            const maxM = Math.max(avgOud || 0, avgNieuw || 0) || 1.8;
            const pxPerM = 180 / maxM; // max visuele hoogte ~ 180px

            const rawBars = [
                { jaar: jaarOud, heightM: avgOud, color: '#1f77b4' },
                { jaar: jaarNieuw, heightM: avgNieuw, color: '#ff7f0e' }
            ];
